...
routeur.get("/books/:id", authentifier(), (req, res) => {
  ...
  if (!livre)
    return res
      .status(404)
      .json({ erreur: "introuvable", message: "Livre inconnu." });
...
routeur.post("/books", ecrivain(), (req, res) => {
  ...
  if (erreurs)
    return res.status(422).json({ erreur: "validation", champs: erreurs });
...
function majLivre(req, res) {
  ...
  if (
    ifMatch !== undefined &&
    Number(String(ifMatch).replace(/"/g, "")) !== actuel.version
  ) {
...
routeur.post("/books/:id/notes", ecrivain(), (req, res) => {
  ...
  const contenu =
    typeof req.body?.contenu === "string" ? req.body.contenu.trim() : "";
...
routeur.delete("/books/:livreId/notes/:noteId", ecrivain(), (req, res) => {
  ...
  if (index === -1)
    return res
      .status(404)
      .json({ erreur: "introuvable", message: "Note inconnue." });
