...
describe('AuthService Suite', () => {
  ...
  it('should authenticate user and store tokens securely on login', async () => {
    ...
    expect(authApi.loginApi).toHaveBeenCalledWith('editeur@booklist.fr', 'editeur123');
  ...
  it('should single-flight concurrent token refreshes', async () => {
    (secureStorage.getSecureItem as jest.Mock).mockResolvedValue('refresh-token-active');
    (authApi.refreshApi as jest.Mock).mockImplementation(
      () =>
        new Promise((resolve) =>
          setTimeout(() => resolve({ accessToken: 'new-acc-tok', expiresIn: 120 }), 50)
        )
    );
  ...
  it('should clear session on logout', async () => {
    ...
    expect(secureStorage.removeSecureItem).toHaveBeenCalledWith('BOOKLIST_REFRESH_TOKEN');
  });
});
